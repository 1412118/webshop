...
function Slider() {
    ...
    const nextSlide = () => {
        if (slideIndex !== dataSliderHome.length - 1) {
            setSlideIndex(slideIndex + 1);
        } else if (slideIndex === dataSliderHome.length - 1) {
            setSlideIndex(0);
        }
    };
    ...
    const prevSlide = () => {
        if (slideIndex !== 0) {
            ...
            setSlideIndex(dataSliderHome.length - 1);
        }
    };
    ...
    useEffect(() => {
        ...
        timeoutRef.current = setTimeout(
            () => setSlideIndex((prevIndex) => (prevIndex === dataSliderHome.length - 1 ? 0 : prevIndex + 1)),
            delay,
        );
